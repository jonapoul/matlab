...
for i = 2 : length(AllTimeDumps);
 ...
 LC = h_proj_3d(DG.r, D.L_C, DG.h, Res, Limits);
 LC = (LC./N);
 LC(LC<0) = 0;
 LC = LC .* Mpc;
 
 % Plot and save
 imagesc(X, Y, log10(LC(1:end, 1:end, Slice)) );
 c = colorbar('YTick',log10(logscale),'YTickLabel',logticks);
 c.Label.String = 'L_{C} (J m^{-3} s^{-1})';
 colormap(flipud(colourmap(1024)));    
 caxis( log10([logscale(1) logscale(length(logscale))]) );
 axis equal
 title(sprintf('Compton Cooling Rate; t=%07.3fMa; nSources=%d; z=%d',TimeDump, NumSources, Slice));
 eval(['print -dpng plots/map/L_C/t=',timeStr,'.png']);
end

% Initialise videos
Video = VideoWriter(['plots/map/L_C/L_C.avi']);
Video.FrameRate = 3;
Video.Quality = 100;
open(Video);
...
for i = 2 : length(AllTimeDumps);
 ...
 FilePath = ['plots/map/L_C/t=',strrep(sprintf('%07.3f',TimeDump),'.','_'),'.png'];
