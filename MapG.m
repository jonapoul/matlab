...
for i = 2 : length(AllTimeDumps);
 ...
 G = h_proj_3d(DG.r, D.G, DG.h, Res, Limits);
 
 % Plot and save
 imagesc(X, Y, log10(G(1:end, 1:end, Slice)) );
 c = colorbar('YTick',log10(logscale),'YTickLabel',logticks);
 c.Label.String = 'G_{total} (J m^{-3} s^{-1})';
 colormap(flipud(colourmap(1024)));    
 caxis( log10([logscale(1) logscale(length(logscale))]) );
 axis equal
 title(sprintf('Total Heating Rate; t=%07.3fMa; nSources=%d; z=%d',TimeDump, NumSources, Slice));
 eval(['print -dpng plots/map/G/t=',timeStr,'.png']);
end

% Initialise videos
Video = VideoWriter(['plots/map/G/G_tot.avi']);
Video.FrameRate = 3;
Video.Quality = 100;
open(Video);
...
for i = 2 : length(AllTimeDumps);
 ...
 FilePath = ['plots/map/G/t=',strrep(sprintf('%07.3f',TimeDump),'.','_'),'.png'];
